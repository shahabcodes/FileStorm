...
fun kindColor(kind: FileKind, colors: com.shahaabapps.filestorm.ui.theme.FsColors): Color =
    when (kind) {
...
fun FileIconView(entry: FsEntry, size: Dp = 40.dp, cornerRadius: Dp = 10.dp) {
    val color = if (entry.isDirectory) {
        com.shahaabapps.filestorm.data.FolderStyles.colorOf(entry.path)?.let { Color(it) }
